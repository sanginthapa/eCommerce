...
?>
<div class="col-12 d-flex"><h3><?php echo $data['product_name']; ?></h3>&nbsp;&nbsp;
<?php
 }
}
?>
 <button type="button" class="btn btn-outline-secondary col-2 mb-2" data-bs-target="#exampleModalToggle" href="#exampleModalToggle" data-bs-toggle="modal"><i class="bi bi-person-plus-fill"></i> Add</button> </div>
<!-- datatable start -->
<!-- datatable start -->
  <table id="table_id" class="display">
   <thead>
    ...
    <tbody>
      <?php
      // $cat_id = $_GET['id'];
      $query="select id,product_name,actual_price,sell_price,img_path,primary_image from products where category_id=$cat_id;";
      // $conn = dbConnecting();
      $req =  mysqli_query($conn,$query) or die(mysqli_error($conn));
      if(mysqli_num_rows($req)>0){
                            while($data = mysqli_fetch_assoc($req)){  ?>
    ...
    <div class="modal-content ms-5">
      ...
      <form action="#" method="post" enctype="multipart/form-data">
        ...
        <div class="card mb-3 ms-3 me-3 mt-5">
          ...
          <span class="m-3">Note:Please upload photo with 400*400 pixel image only. Must be transperent(no background).</span>
        </div>
