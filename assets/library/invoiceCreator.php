...
// function get_my_coupon_code($ref_id){
...
function print_My_Invoice($get_ref_id){
        $html = '';
        $html .='';
    ...
    if ($row > 0) {
        ...
        $html .='<div>Your order is confirmed. Thank you for purchasing products from our store. Your Purchase Details are as follows:</div>';
        $html .='';
        $payment_part='';
        $table_header_part='';
        $table_body_part='';
	...
	$html .='<br><div>Order Reference Code: <strong>'.$get_ref_id.'</strong></div> <div style="margin:10px 0px;">You can check your order here <a style="background: #0498be;padding: 8px;color: white;border-radius: 4px;" href="https://ultimanepal.com/cartOut.php?refID='.$get_ref_id.'">Check</a></div>';
    $html .='</div>';?>
        <?php 
    // <!--<textarea>-->
        //echo $html; 
