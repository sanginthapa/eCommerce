...
<!-- <div class="container mt-4 mb-2">
  <div class="h1 fw-bolder text-white"> <span class="border-bottom border-3">Search Result</span></div>
</div> -->
<div style="background-color:black;" class="row d-flex dynamic-product-container ms-3">
  <?php
  $getproductName = 0;
  if (isset($_GET['name'])) {
...
// $sql = "SELECT id,product_name,actual_price,sell_price,img_path,primary_image,secondary_image,url_link FROM products";
  $sql = "SELECT id,product_name,LEFT(product_name,16)as trunkName,ROUND(actual_price,0) as actual_price,ROUND(sell_price,0)as sell_price,img_path,primary_image,secondary_image,url_link FROM products where `product_name`= '$getproductName' ";
//   echo $sql;
  $conn = connectdb();
...
//product card layout 
    ...
    echo '<h1 class="text-white"> Nothing to show </h1>';
  }
